...
function validateStore(value) {
  ...
  // Additive migration keeps existing v1 data files readable after upgrades.
  if (!Array.isArray(value.dailyUsage)) value.dailyUsage = [];
  if (!Array.isArray(value.quotaUsage)) value.quotaUsage = [];
  if (!Array.isArray(value.generationJobs)) value.generationJobs = [];
  if (!Array.isArray(value.captchaUsage)) value.captchaUsage = [];
  if (!Array.isArray(value.loginFailures)) value.loginFailures = [];
  const expectedArrays = ['users', 'sessions', 'chats', 'messages', 'dailyUsage', 'quotaUsage', 'generationJobs', 'authRateLimits', 'captchaUsage', 'loginFailures'];
