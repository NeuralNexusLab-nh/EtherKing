...
const QUOTA_WINDOWS = Object.freeze({
  fiveHour: Object.freeze({ label: '5-hour limit', capacity: 131072, durationMs: 5 * 60 * 60 * 1000 }),
  weekly: Object.freeze({ label: 'Weekly limit', capacity: 1048576, durationMs: 7 * 24 * 60 * 60 * 1000 })
});
...
function resetRecord(record, definition, now) {
  record.capacity = definition.capacity;
...
function ensureRecord(data, userId, windowName, now) {
  ...
  if (active.length) {
    const legacyCapacity = windowName === 'fiveHour' ? 100 : 1000;
    const spentPoints = active.reduce((total, item) => {
      const previousCapacity = Number.isFinite(item.capacity) ? Number(item.capacity) : legacyCapacity;
      return total + (previousCapacity - Math.min(previousCapacity, Number(item.remainingPoints)));
    }, 0);
    record.capacity = definition.capacity;
...
function chargeQuota(data, userId, pointsValue, now = Date.now()) {
  const points = Number(pointsValue);
  if (!Number.isFinite(points) || points < 0) throw new Error('Quota charge is invalid.');
  const records = Object.keys(QUOTA_WINDOWS).map((windowName) => ensureRecord(data, userId, windowName, now));
  for (const record of records) {
    record.remainingPoints -= points;
    record.updatedAt = new Date(now).toISOString();
  }
  return Object.fromEntries(records.map((record) => [record.window, serializeRecord(record, QUOTA_WINDOWS[record.window])]));
}
